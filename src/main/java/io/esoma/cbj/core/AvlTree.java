...
public class AvlTree<E extends Comparable<E>> {
	...
	/**
	 * Checks if the given node contains the requested element, and if not,
	 * recursively checks its children based on comparison.
	 * 
	 * @param n the node to check
	 * @param e the element to search
	 * @return true if the node or one of its children has the given element, or
	 *         false otherwise
	 */
	 ...
	 * @return an integer representing the height of host after appending the new
	 *         node, or -1 if the new node is not inserted
	 */
	...
	/**
	 * Deletes the given element from the tree if present. If it has one child, this
	 * child will be promoted to take its place. If the node has two children, its
	 * value will be replaced by its closest successor, and then the successor will
	 * be deleted instead. Rebalancing will be performed if necessary once all
	 * orphan nodes are placed.
	 * 
	...
	/**
	 * Cuts the node off from the tree if it contains the same element as the input.
	 * This is recursively called on the appropriate child node until the target
	 * node is found or it becomes apparent that the element is not present.
	 * 
	 * @param n the node to start
	 * @param e the target value to be cut
	 * @return the height of the node once the target has been cut from the tree, or
	 *         -2 if the target is not found
	 */
	...
	/**
	 * Checks the balance factor of the given node, and if the constraint is
	 * violated, triggers the operation to fix the tree structure. All parent nodes
	 * will also be checked once the current node is in shape.
	 * 
	 * @param n the node to check
	 */
	...
	/**
	 * Performs an in order traversal of the given node and its children and appends
	 * the elements to the list in ascending order.
	 * 
