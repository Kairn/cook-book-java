...
public class GreatestCommonDivisor {
	...
	/**
	 * Calculates the GCD between two integers using the Euclidean algorithm. The
	 ...
	 * its difference with the smaller number. The operation repeatedly replaces the
	 * larger integer of the two with the remainder of dividing it by the smaller
	 * integer until the smaller integer is a factor of (or equal to) the larger
	 * one.
	 * 
	...
	/**
	 * Computes the largest GCD with the given parameters and a cached array. This
	 * method is recursive and built on the principle of dynamic programming. The
	 * core logic is to enumerate every element of the array and decide if the
	 * current element should be a part of the final combination. Caching is used to
	 * quickly look up already computed values. However, this does not scale well
