 ...
 * derived from merge sort and insertion sort, designed to perform well on many kinds of real-world
 * data. It was implemented by Tim Peters in 2002 for use in the Python programming language. It is
 * also used to sort arrays of non-primitive type in Java SE 7, on the Android platform, in GNU
 * Octave, on V8, and Swift. Timsort was designed to take advantage of runs of consecutive ordered
 * elements that already exist in most real-world data, natural runs. In a nutshell, the main
 * routine marches over the array once, left to right, alternately identifying the next run, then
 * merging it into the previous runs "intelligently".
 *
 * @author Eddy Soma
 */
public class TimSort {
  ...
  /**
   * Checks the merge condition and merges the top runs in the stack if necessary. It is recursive
  ...
  /**
   * Merges two runs starting at the left bound. Assumes the left side is shorter. Automatically
   * puts the left run into temporary memory and starts pairing mode.
   *
  ...
  /**
   * Merges two runs starting at the right bound. Assumes the right side is shorter. Automatically
   * puts the right run into temporary memory and starts pairing mode.
   *
   * @param array the source array
   * @param bin the beginning index
   * @param end the ending index
