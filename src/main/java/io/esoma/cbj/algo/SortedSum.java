...
/**
 * Class for studying the Sorted Sum problem. The task is to efficiently compute the sum of the
 * first i elements multiplied by its index (1 based) after sorting. The function should return the
 * total of such weighted sums for all i (1 <= i <= n). A naive algorithm will simply sort every
 * prefix array, compute the weighted sum, and add it to the total value. Typically, a modulo
 * operation will be performed on the total when the potential value exceeds the maximum of a 32-bit
 * number.
 *
 * @author Eddy Soma
 */
