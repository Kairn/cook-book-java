...
/**
 * Class for implementing the PKCS#7 padding scheme commonly used in cryptography to format
 * irregularly sized blocks of text. Padding is in whole bytes, and the number of padding cannot
 * exceed 255. Padding is produced by appending N bytes of value N to the end of the block, where N
 * is the number of missing bytes.
 *
 * @author Eddy Soma
 */
public class PKCS7Padding {
  ...
  /**
   * Removes the padding values from a block of bytes based on PKCS#7 assuming the block contains
   * padding values. If the block does not conform to the padding rules, it will be returned as is
   * if parsing is lenient.
   *
