...
/**
 * Class that focuses on cracking the unknown secret text behind a less silly ECB encryption oracle
 * which prepends some unknown bytes to the text before encrypting.
 *
 * @author Eddy Soma
 */
public class EcbOracleAttackPlus {
  ...
  public static int findAttackPrefixLength(EncryptionScheme scheme) {
    ...
    byte[] zeroCipher =
        Arrays.copyOfRange(scheme.encrypt(ATTACK_REPO), BLOCK_SIZE * 2, BLOCK_SIZE * 3);
