...
/**
 * Class for implementing an exploit against fixed-nonce CTR encryption scheme.
 * By evaluating a large number of cipher texts produced by the same CTR
 * encryption of the same nonce, we can make informative guesses on the key
 * stream bytes.
 * 
 * @author Eddy Soma
 *
...
public class CtrFixedNonceAttack {
	...
	/**
	 * Enumerates the cipher texts and tests each byte on each position to obtain a
	 * good guess of the key stream given the ciphers are produced by CTR with a
	 * fixed-nonce. The strategy for guessing is based on the assumption that the
	 * plain texts are all common English sentences.
	 * 
	...
	public static byte[] loadAndGuess() {
		...
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(loader.getResourceAsStream(TEST_FILE)))) {
